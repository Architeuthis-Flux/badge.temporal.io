...
function appFolder(path) {
  const match = path.match(/^registry\/community_apps\/([^/]+)\//);
  return match ? match[1] : null;
}

function isCommunitySubmissionPath(path) {
  return (
    path.startsWith("registry/community_apps/") ||
    path === "registry/community_apps.json" ||
...
function needsDocsReview(path) {
  ...
  if (path === "registry/README.md") return false;
...
function summarizeStaticReview(files, contents) {
  ...
  const hasCommunityAppChange =
    appFolders.length > 0 ||
    changedPaths.some((path) => path === "registry/community_apps.json");
  ...
  if (hasCommunityAppChange && appFolders.length === 0) {
    errors.push("No app folder under registry/community_apps/<app>/ was changed.");
  }
  ...
  for (const folder of appFolders) {
    const prefix = `registry/community_apps/${folder}/`;
  ...
  if (docsSensitivePaths.length && docPaths.length === 0) {
    warnings.push(
      "This PR changes firmware, MicroPython, Ignition, registry, or badge app behavior but does not update docs. Reviewers should confirm whether docs, README files, or user-facing guidance need changes."
    );
  } else if (docsSensitivePaths.length) {
    notes.push(`Docs-related files changed: ${docPaths.join(", ")}.`);
  }

  if (hasCommunityAppChange && !changedPaths.includes("registry/community_apps.json")) {
    warnings.push("registry/community_apps.json was not changed; run python3 firmware/scripts/generate_startup_files.py before merging.");
  }
...
The badge is an ESP32-S3 device with a 128x64 OLED, 8x8 LED matrix, joystick/buttons, haptics, IR, WiFi, and an embedded MicroPython runtime. Community apps are installed from registry/community_apps/<app>/ into /apps/<app>/ on the badge.
...
- Whether firmware, MicroPython, Ignition, registry, or badge app behavior changes need matching docs updates.
