...
- `docs/` contains the static documentation site. Vercel serves this directory
  directly through the root `vercel.json` config, and docs CI runs Lighthouse
  against the static site.
...
- `ignition/` is the Temporal-powered flashing tool for one badge or many
  badges.
- `community_apps/` holds public Community App submissions; `release-assets/`
  documents firmware release artifacts.
...
- Ignition is the default flashing path; recommend
  `./start.sh --latest-release` for public factory flashing unless the user is
  intentionally building from source.
