...
want. Pulls from the release-hosted `community_apps.json` generated from
`community_apps/`. Useful for users who can't run PlatformIO
themselves.
...
|--------|------|----------|----------|
| v2 | `community_apps.json` GitHub Release asset | `generate_startup_files.py` from `community_apps/` | current firmware (Community Apps) |
| – | `firmware/data/manifest.json` | `generate_startup_files.py` | `badge_sync.py`, JumperIDE |

New entries go into the generated v2 release asset. Community app file lists
are inlined in that JSON; stale per-app `manifest.json` files are removed by
the generator.
