...
| `identity/` | Per-badge state. `BadgeUID.{cpp,h}` (eFuse MAC + internal UUID v5), `BadgeInfo.{cpp,h}` (editable NVS contact card + legacy `/badgeInfo.json` migration), `BadgeVersion.h`. |
...
editable contact card lives in NVS (`badge_info`) and can be changed from the
Badge Info screen or over USB serial with `badge.contact()` /
`badge.set_contact({...})`.
