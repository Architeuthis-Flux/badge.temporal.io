...
1. Downloads the latest GitHub Release factory image for the recommended public flashing path
2. Detects all connected badges over USB
3. Builds firmware via PlatformIO when you choose a source build path
4. Prepares a fresh default filesystem image when building, then starts one child workflow per detected badge
5. Flashes firmware + apps to all badges in parallel with one direct esptool upload per badge and auto-retry
6. Shows per-badge live status (Resolving USB -> Writing firmware + apps -> Verify boot -> Sync clock -> Done), with esptool progress in Temporal activity heartbeat details
7. Includes bounded boot and clock-sync logs in the verification activity results
8. Asks for confirmation before writing to connected badges (`-y` skips the prompt when you are ready)
9. Concurrency cap: up to 32 badges flashing at once
...
```bash
./start.sh --latest-release          # recommended: download + flash latest GitHub factory image
...
./start.sh --release-tag v1.0.0      # download + flash a specific release factory image
...
Ignition can flash the same factory image published on GitHub Releases. This is
the recommended public flashing path and the easiest way to confirm the release
...
The command downloads `replay2026-factory-16MB.bin` from the latest
`temporal-community/badge.temporal.io` release, caches it under
`~/.cache/replay2026-badge/releases/`, and flashes it through the normal
Temporal workflow. Use `--release-tag v1.0.0` to flash a specific release, or
`--force-download` to refresh the cached image. If GitHub rate limits the API
or you are testing a private release, set `GITHUB_TOKEN` before running
Ignition.
